#include "INCPNTStaticDraw2Title.hlsli"

Texture2D<float4> g_texture : register(t0);
// 深度マップ
Texture2D g_DepthMap : register(t1);
Texture2D g_TADrawMap : register(t2);
...
float4 main(PSPNTInputShadow input) : SV_TARGET
{
	...
	float2 TATexCoords;
	TATexCoords.x = 0.5f + (input.TASpacePos.x / input.TASpacePos.w * 0.5f);
	TATexCoords.y = 0.5f - (input.TASpacePos.y / input.TASpacePos.w * 0.5f);
