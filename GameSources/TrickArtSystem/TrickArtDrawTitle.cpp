...
namespace basecross {
	IMPLEMENT_DX11_CONSTANT_BUFFER(CBTrickArtTitle)
	IMPLEMENT_DX11_VERTEX_SHADER(VSTrickArtTitle, App::GetApp()->GetShadersPath() + L"VSTrickArtTitle.cso")
	IMPLEMENT_DX11_PIXEL_SHADER(PSTrickArtTitle, App::GetApp()->GetShadersPath() + L"PSTrickArtTitle.cso")

	struct TrickArtDrawTitle::Impl :public DrawObjectBase {
		//頂点変更する場合のメッシュ（オリジナル）
	...
	TrickArtDrawTitle::TrickArtDrawTitle(const shared_ptr<GameObject>& GameObjectPtr)
		:SmBaseDraw(GameObjectPtr), pImpl(new Impl()), m_flg(false)
	{}
	TrickArtDrawTitle::~TrickArtDrawTitle() {}

	void TrickArtDrawTitle::OnCreate() {
		//ライティングのみだと極端になるので調整
		SetEmissive(bsm::Col4(0.5f, 0.5f, 0.5f, 0.0f));
		SetDiffuse(bsm::Col4(0.6f, 0.6f, 0.6f, 1.0f));
	}

	void TrickArtDrawTitle::OnDraw() {
	...
	void TrickArtDrawTitle::DrawStatic2(const MeshPrimData& data) {
		auto Dev = App::GetApp()->GetDeviceResources();
		...
		//シェーダの設定
		//頂点シェーダ
		pD3D11DeviceContext->VSSetShader(VSTrickArtTitle::GetPtr()->GetShader(), nullptr, 0);
		//インプットレイアウトの設定
		pD3D11DeviceContext->IASetInputLayout(VSTrickArtTitle::GetPtr()->GetInputLayout());
		//ピクセルシェーダ
		pD3D11DeviceContext->PSSetShader(PSTrickArtTitle::GetPtr()->GetShader(), nullptr, 0);
		//個別処理
		...
		//コンスタントバッファの更新
		pD3D11DeviceContext->UpdateSubresource(CBTrickArtTitle::GetPtr()->GetBuffer(), 0, nullptr, &SmCb, 0, 0);
		//コンスタントバッファの設定
		ID3D11Buffer* pConstantBuffer = CBTrickArtTitle::GetPtr()->GetBuffer();
	...
	void TrickArtDrawTitle::SetConstants2(SimpleConstants& SmCb, const MeshPrimData& data) {
		//行列の定義
		...
		//ビューと射影行列を得る
		ViewMat = XMMatrixLookAtLH(m_defPos, at, bsm::Vec3(0, 1.0f, 0));
		//転置する
		ViewMat.transpose();
		//転置する
